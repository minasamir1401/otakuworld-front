...
export default function ShowPage() {
  ...
  const fetchShowDetails = async () => {
    setLoading(true);
    try {
      const response = await fetch(`/api/show/details?slug=${slug}`);
      const data = await response.json();
      if (data.success && data.show) {
        setShow(data.show);
        // Default to first season if available
        if (data.show.seasons && data.show.seasons.length > 0) {
          setActiveSeason(data.show.seasons[0]);
        }
      }
    } catch (error) {
      console.error('Failed fetching show details:', error);
    } finally {
      setLoading(false);
    }
  };
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center py-40 gap-4 glass-panel">
        ...
        <p className="text-text-muted text-sm font-black">{t('loadingShow')}</p>
      </div>
    );
  }
  ...
  if (!show) {
    return (
      <div className="glass-panel p-16 text-center text-text-muted space-y-4">
        <p className="text-lg font-bold">{t('notFoundShow')}</p>
        <button 
          onClick={() => router.push('/')}
  ...
  return (
    <div className={`space-y-10 ${isAr ? 'text-right' : 'text-left'}`}>
      ...
      <div className="relative rounded-3xl overflow-hidden h-[340px] md:h-[480px] border border-border-glass shadow-2xl bg-bg-primary">
        <img 
          src={getProxyImageUrl(show.poster)} 
          alt="Backdrop"
        ...
        <div className={`absolute bottom-6 ${isAr ? 'right-6 md:right-12' : 'left-6 md:left-12'} flex flex-col md:flex-row items-end gap-6`}>
          <div className="hidden md:block w-36 aspect-[3/4] rounded-2xl overflow-hidden border-2 border-white/10 shadow-2xl scale-105 transform translate-y-2">
            <img src={getProxyImageUrl(show.poster)} alt={show.title} referrerPolicy="no-referrer" className="object-cover w-full h-full" />
          </div>
          <div className={`space-y-2.5 pb-2 ${isAr ? 'text-right' : 'text-left'}`}>
            <div className="flex flex-wrap items-center gap-2">
              {show.genres && show.genres.split(',').map((g, idx) => (
            ...
            {show.arabicTitle && (
              <p className="text-text-muted text-xs font-semibold">{show.arabicTitle}</p>
            )}
          </div>
        </div>
      </div>
            ...
            <div className="space-y-4 text-xs font-bold divide-y divide-border-glass">
              <div className="flex justify-between py-3 items-center">
                ...
                <span className="font-black text-accent-primary bg-accent-primary/10 px-2.5 py-1 rounded-lg">
                  ⭐ {show.rating ? parseFloat(show.rating).toFixed(1) : '0.0'}
                </span>
              </div>
              <div className="flex justify-between py-3 items-center">
                <span className="text-text-muted">{t('typeLabel')}</span>
                <span className="text-text-primary font-extrabold">{show.type || 'TV'}</span>
              </div>
              <div className="flex justify-between py-3 items-center">
                ...
                <span className="font-extrabold text-emerald-550 bg-emerald-500/10 border border-emerald-500/20 px-2.5 py-1 rounded-lg">
                  {show.status === 'مستمر' ? t('statusOngoing') : show.status === 'منتهي' ? t('statusCompleted') : (show.status || t('statusOngoing'))}
                </span>
              </div>
              <div className="flex justify-between py-3 items-center">
                <span className="text-text-muted">{t('yearLabel')}</span>
                <span className="text-text-primary font-extrabold">{show.year || 'N/A'}</span>
              </div>
              <div className="flex justify-between py-3 items-center">
                <span className="text-text-muted">{t('durationLabel')}</span>
                <span className="text-text-primary font-extrabold">
                  {show.duration ? show.duration.replace('دقيقة', t('durationSuffix')).replace('د', t('durationSuffix')).replace('m', t('durationSuffix')).trim() + ' ' + (isAr ? 'دقيقة' : 'min') : ('24 ' + (isAr ? 'دقيقة' : 'min'))}
                </span>
              </div>
            </div>
          ...
          {show.seasons && show.seasons.length > 0 ? (
            <div className="glass-panel p-6 space-y-6">
              ...
              <div className="flex items-center justify-between border-b border-border-glass pb-4">
                ...
                <span className="text-xs text-text-muted font-extrabold">{t('seasonsCountLabel')} {show.seasons.length}</span>
              </div>
